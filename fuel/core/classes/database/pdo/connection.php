...
//		$this->_schema = new \Database_Drivername_Schema($name, $this);
	 ...
	 * @throws \Database_Exception
	 */
	public function query($type, $sql, $as_object, $multi_query_result_count = null)
	{
		// Make sure the database is connected
		$this->_connection or $this->connect();
		...
		}elseif ($type === \DB::MULTI_QUERY)
        {
            ...
            $result_count = 0;
            do{
                if( $result_count++ == $multi_query_result_count) break;

                $fetchObj = null;
                if ($as_object === false)
                {
                    $fetchObj = $result->fetchAll(\PDO::FETCH_ASSOC);
                }
                elseif (is_string($as_object))
                {
                    $fetchObj = $result->fetchAll(\PDO::FETCH_CLASS, $as_object);
                }
                else
                {
                    $fetchObj = $result->fetchAll(\PDO::FETCH_CLASS, 'stdClass');
                }

                if( $fetchObj != null )
                    $multi_result[] = new \Database_Result_Cached($fetchObj, $sql, $as_object);
