<?php
	...
	class Gmail
	{
		 ...
		 */
		public function __construct($username = null, $password = null, $host = null, $port = 993, $encryption = false)
		{
			$enc = '';
			if ($encryption != null && isset($encryption) && $encryption == 'ssl')
			{
				$enc = '/novalidate-cert/imap/ssl';
			}
			else
			{
				if ($encryption != null && isset($encryption) && $encryption == 'tls')
				{
					$enc = '/novalidate-cert/imap/tls';
				}
			}

			$this->mailbox = '{' . $host . ':' . $port . $enc . '}';
